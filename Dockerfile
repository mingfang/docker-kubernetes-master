...
#Etcd
RUN wget -O - https://github.com/coreos/etcd/releases/download/v3.2.8/etcd-v3.2.8-linux-amd64.tar.gz | tar zx
RUN mv /etcd* /etcd && \
...
#Influxdb
RUN wget https://dl.influxdata.com/influxdb/releases/influxdb_1.3.6_amd64.deb && \
    dpkg -i influxdb*.deb && \
    rm influxdb*.deb
