...
#Etcd
RUN wget -O - https://github.com/coreos/etcd/releases/download/v3.2.9/etcd-v3.2.9-linux-amd64.tar.gz | tar zx
RUN mv /etcd* /etcd && \
...
#Influxdb
RUN wget https://dl.influxdata.com/influxdb/releases/influxdb_1.3.7_amd64.deb && \
    dpkg -i influxdb*.deb && \
    rm influxdb*.deb
...
#Kubernetes
RUN wget -P /usr/local/bin https://storage.googleapis.com/kubernetes-release/release/v1.8.4/bin/linux/amd64/kube-apiserver
RUN wget -P /usr/local/bin https://storage.googleapis.com/kubernetes-release/release/v1.8.4/bin/linux/amd64/kube-controller-manager
RUN wget -P /usr/local/bin https://storage.googleapis.com/kubernetes-release/release/v1.8.4/bin/linux/amd64/kube-scheduler
